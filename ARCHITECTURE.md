...
The frontend follows a session-first workflow. It loads only the session summary list on landing, then fetches tab data for the selected session on demand. The Graph tab renders topic nodes in a dependency-free SVG graph and shows memory nodes only for the selected topic, keeping large sessions readable while preserving node details, lifecycle badges, curved directional edges, and client-side filters for node type, tags, and lifecycle state. The Tables tab is a read-only database inspection surface with original `mg_*` table names, pagination, and expandable cells for long values. Invoke Preview renders the shared invocation plan as context selection, structured messages, raw memory context, and retrieval explanation. Manual refresh controls let developers reload the session list or the active tab without restarting Studio.
