...
`TopicCluster` is session-scoped organizational metadata. A stable topic references zero or one cluster; episodes and facts continue to reference the stable topic. Cluster membership is stored separately from topic edges and cannot participate in graph traversal or retrieval ranking.

`TopicClusterAssigner` runs after either ingestion path commits. It proposes a broad domain from the stable topic's label and summary, embeds that domain separately, and verifies a bounded shortlist from the existing catalog before creating a domain. Canonical labels and verified aliases receive priority in the shortlist. Ambiguity produces an unassigned topic rather than a new catch-all cluster. Existing assignments remain stable, while evaluated revisions and cooldowns bound repeated provider work. A per-run topic limit defers excess topics to the resumable backfill.

`TopicClusterStore` serializes final writes with a session advisory lock. The decision includes both the topic revision and a digest of the observed cluster catalog. Both must still match inside the transaction; otherwise the assigner refreshes and retries once outside the lock. Cluster creation and membership are atomic, and the database enforces canonical-label uniqueness and same-session membership. No provider call runs inside the transaction. Cluster deletion and session clearing use the same lock.

Domain catalogs are read once per classification batch and refreshed after creation, alias changes, or a stale decision. The small, session-owned catalog is ranked locally; only the configured top candidates reach the LLM. Domain embeddings are not episode centroids and do not change when another episode is assigned. Large catalogs may later warrant server-side vector candidate search; there is deliberately no cluster traversal or retrieval boost in this phase.

Retrieval hydrates compact cluster metadata for selected topic/session pairs after ranking and token allocation, including topic references from selected episodes. This batch query reads current memberships rather than cached cluster IDs. Cluster edits do not invalidate candidate caches or change prompts. Studio uses cluster membership for collapsible topic navigation and presents an Unclustered group; graph edges retain their existing meaning.
