...
- `memo-grafter studio`: launch a local Studio host for browsing sessions, inspecting graph/table/prompt-preview state, and applying the supported topic suppression action.
...
The frontend follows a session-first workflow. It loads only the session summary list on landing, then fetches tab data for the selected session on demand. The Graph tab renders topic nodes in a dependency-free SVG graph and shows memory nodes only for the selected topic, keeping large sessions readable while preserving node details, lifecycle badges, curved directional edges, and client-side filters for node type, tags, and lifecycle state. The Tables tab is a read-only database inspection surface with original `mg_*` table names, pagination, and expandable cells for long values. Prompt Preview runs read-only graft or recall simulations for a query and displays the exact generated system prompt plus token usage. Manual refresh controls let developers reload the session list or the active tab without restarting Studio.

Prompt Preview is backed by `src/studio/StudioPreviewService.ts` and exported through `memo-grafter/studio`. Studio resolves runtime settings from `mg.config.ts`; `memo-grafter init` scaffolds an OpenAI-compatible embedder that activates when `OPENAI_API_KEY` is set and uses `MEMO_GRAFTER_EMBEDDING_MODEL` or `text-embedding-3-small`. Without an embedder, the Prompt Preview tab remains visible but reports that preview is unavailable; database browsing, graph inspection, table inspection, and supported lifecycle actions remain usable without any provider SDK.
