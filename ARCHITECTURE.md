...
The recall path embeds the query, retrieves the nearest 40 active memory candidates by default without an absolute similarity cutoff, filters decayed, superseded, forgotten, or suppressed-topic memories, combines each fact's similarity and confidence into a confidence-weighted retrieval score, and groups facts by parent topic node. Topic blocks are selected adaptively using their score relative to the best block and adjacent score gaps, then formatted within the fact, topic, and token limits.
...
When `cache` config is provided, `MemoGrafter` owns one shared Redis client for recall caching. `RetrieverPipeline` uses that client only around raw candidate generation, caching candidates before ranking, adaptive selection, and prompt assembly. Cache keys include the session ID, candidate limit, candidate-strategy version, recall scope, tag mode, normalized tag list, and a short hash of the embedding. TTL is clamped to 60-120 seconds, defaulting to 90 seconds. Redis errors are logged as warnings and retrieval falls back to the store. Successful lifecycle changes clear recall cache keys so stale cached results do not reintroduce forgotten or suppressed memory.
