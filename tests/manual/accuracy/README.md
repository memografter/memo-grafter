...
`accuracy:retrieval-impact` is a deterministic, provider-free A/B regression case. It compares the former similarity-cutoff/top-10 flow with top-40 candidate generation and adaptive selection, and fails unless the new path recovers a relevant fact below the former similarity cutoff that the legacy path discards.
