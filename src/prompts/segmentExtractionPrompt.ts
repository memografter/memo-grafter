...
export function buildSegmentExtractionPrompt(messages: Message[], labelHint?: string, sourceType: "conversation" | "note" | "document" | "code" = "conversation"): string {
  ...
  return [
    ...
    '      "quality": { "explicitness": 0.95, "sourceReliability": 0.8, "stability": 0.8, "salience": 0.9 },',
    '      "provenance": {',
    ...
    "",
    "Quality scoring (each numeric dimension must be between 0 and 1):",
    "- explicitness: direct statement or confirmation 0.9-1; supported inference 0.5-0.8; speculation below 0.5. This does not establish truth.",
    "- sourceReliability: trustworthiness for this specific claim. Firsthand personal preferences can score 0.8-0.9; unknown document authority is 0.5; unreliable hearsay below 0.5. Never invent external verification.",
    "- stability: expected validity over time. Temporary task or current state 0.1-0.3; project decision 0.5-0.8; enduring constraint 0.8-0.95. Do not score age.",
    "- salience: expected usefulness beyond this exchange. Incidental detail 0.1-0.3; reusable context 0.5-0.7; important constraint, decision, or recurring preference 0.8-0.95. Do not score relevance to a current query.",
    "- Use 0.5 when a dimension is unknown. Score dimensions independently, never copy one score into all four.",
    ...
    '      "quality": { "explicitness": 0.95, "sourceReliability": 0.8, "stability": 0.3, "salience": 0.8 },',
    '      "provenance": { "speaker": "user", "message_indexes": [1], "extraction_method": "explicit" }',
    "    }",
    "  ]",
    "}",
